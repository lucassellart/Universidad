...
Nodo * rotacionDerecha(Nodo * y) {
    ...
    // Asignaciones:
    Nodo * x = y->h_izq;         // Hijo izquierdo de y
    Nodo * T2 = x->h_der;        // Subárbol derecho de x
...
Nodo * rotacionIzquierdaDerecha(Nodo * n) {     // Puntero al nodo desbalanceado, para modificar sus subárboles

    n->h_izq = rotacionIzquierda(n->h_izq);     // Rotación simple a la izq. sobre el hijo izq. Corregir el desbalance en el hijo izq. de 'n'

    return rotacionDerecha(n);                  // Rotación simple a la derecha sobre el nodo actual
}
